...
DeviceID <- paste("Sample3 ",Temp,"k ",ChannelLength,"u ","C3  Saturation Transfer.txt",sep="") #批量组合文件名
...
for (i in 1:10){assign(IDhalfTemp[i],eval(parse(text = SatTemp[i]))$Squ_ID)}
VbgSat <- eval(parse(text = SatTemp[1]))$Vg
...
write.csv(Output,file=paste("PENSample3 corrected with Vth and Capacity W=",ChannelLength, sep = ""))
